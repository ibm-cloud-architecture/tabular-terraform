...
| Public Gateway | Created from Public Gateway name (gatewayname:fipname where fipname is optional) on subnet.  Mapping to Public Gateway resource: name, vpc, zone, and floating_ip.id are utilized, but floating_ip.address, resource_controller_url and timeouts are not utilized. |
| Rules | Protocols are in the form icp:type:code or tcp:port_min:port_max or udp:port_min:port_max or if protocol is not specified the protocol is defined as ALL. |
