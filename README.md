...
| Sheets | Asterisk in column name denotes a required field, column names are generally same as resource arguments with some changes for spreadsheet clarity, uniqueness, etc, and columns are subject to change as arguments are added or deprecated but there is no upgrade from previous sheet to changed sheet. |
